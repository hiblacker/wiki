...
let basePath = path.resolve(`./juejin/`)
// 获取所有章节
function getAll() {
    HTTP({ url: list + token, data: { booklet_id } }).then(async ({ data }) => {
        console.log(data)
        basePath += `/${data.data.booklet.base_info.title.replace(/\s/g, '')}/`
        // 写readme
...
async function saveReadme(content) {
    ...
    fs.writeFile(basePath + `/README.md`, content, err => {
        if (err) {
            console.error(err)
            return
        }
...
function saveArticle(section_id, k) {
    return HTTP({ url: url + token, data: { section_id } }).then(async ({ data }) => {
        const title = data.data.section.title.replace(/\s/g, '')
        ...
        fs.writeFile(`${path1}/${k}.md`, md, err => {
            if (err) {
                console.error(err)
                return
            }
        ...
        fs.writeFile(`${path2}/${k}.md`, data.data.section.content, err => {
            if (err) {
                console.error(err)
                return
            }
